...
#define dhcpDelay 60 * 60 * 1000          //60 minute DHCP timer
...
uint16_t sCal[3][4] = {                   //Swell calibration table
...
void setup() {
  ...
  pinMode(i2cReset, OUTPUT);            //GPIO used to drive the resset (run) pin on the slave scanners
  digitalWrite(i2cReset, 1);
  ...
  if (i2caddr == 0) {
    //*********************************************************************************************
    //****                      Send a hardware reset on I2C BUS                               ****
    //*********************************************************************************************
    if (debug == true) Serial.println("Sending Scanner BUS Reset");
    digitalWrite(i2cReset, 0);          //Send reset to slave devices
    digitalWrite(LED_BUILTIN, ledOff);
    delay(200);
    digitalWrite(i2cReset, 1);          //Release reset to allow slave devices to restart
    digitalWrite(LED_BUILTIN, ledOn);
    delay(5000);                        //Allow slave devices time to re-boot
  ...
  if (kbtype < 3) {  //No need to do anything here for illuminated pistons
    ...
    int ii;
    for (ii = 0; ii < 8; ii++) {
      pinMode(pxlte[ii], INPUT_PULLUP);
    }
    for (ii = 8; ii < 16; ii++) {
      pinMode(pxlte[ii], OUTPUT_12MA);
      digitalWrite(pxlte[ii], HIGH);
    }
  }
  ...
  if (i2caddr == 0) scanning = true;                          //Set the flag to start Ethernet ot USB scanning on Core 1
}


//*********************************************************************************************
//****                Core 1 handles all midi receive and transmit tasks                   ****
//*********************************************************************************************
...
void loop1() {                                                //Core 1 main loop
  //*********************************************************************************************
  //****                  If it's time send and receive Midi messages                        ****
  //*********************************************************************************************
  if (noeth == false) {
    if (millis() < dhcpTime) dhcpTime = 0;
    if (millis() > dhcpTime + dhcpDelay) {                    //check for DHCP renew once per hour
      dhcpTime = millis();
      Ethernet.maintain();
    }
  }
...
void loop() {
  ...
  if (kbtype == 0 || kbtype == 2) {
    ...
    if (micros() > kbdTime + kbdDelay) {        //Is it time to scan the keyboard?
      kbdTime = micros();                       //Store time keyboard scan starte
  ...
  if (millis() > LED_BUILTINTime + LedDelay) {
    digitalWrite(LED_BUILTIN, ledOff);          //Turn LED off after a shart delay
  }
}
...
int scanRow(byte kbl[3][128], int r) {
  ...
  if (kbtype == 2) {
    ...
    kbl[0][ki + 1] = digitalRead(pxlte[1]);
    kbl[0][ki + 2] = digitalRead(pxlte[2]);
    kbl[0][ki + 3] = digitalRead(pxlte[3]);
    kbl[0][ki + 4] = digitalRead(pxlte[4]);
    kbl[0][ki + 5] = digitalRead(pxlte[5]);
    kbl[0][ki + 6] = digitalRead(pxlte[6]);
    kbl[0][ki + 7] = digitalRead(pxlte[7]);
  } else {  //Invert mechanical switches
    kbl[0][ki] = !digitalRead(pxlte[0]);
    kbl[0][ki + 1] = !digitalRead(pxlte[1]);
    kbl[0][ki + 2] = !digitalRead(pxlte[2]);
    kbl[0][ki + 3] = !digitalRead(pxlte[3]);
    kbl[0][ki + 4] = !digitalRead(pxlte[4]);
  ...
  if (r == 0) {
    digitalWrite(pxlte[15], HIGH);
  } else {
    digitalWrite(pxlte[7 + r], HIGH);
  }
  digitalWrite(pxlte[8 + r], LOW);
  ...
  for (keyidx = k1; keyidx < k2; keyidx++) {
    ...
    if (kbl[2][keyidx] == 1) {  //Is note aleady on?
      ...
      if (kbl[1][keyidx] == 1) {  //Is key off debounce done ?
        //Send note off midi data to master
      ...
      if (kbl[1][keyidx] == 2) {                            //Has de-bounce completed
        //Send note on midi data to master
...
void receiveEvent(int howMany) {
  while (1 < Wire.available()) {                            //loop through all but the last
    rxa = Wire.read();                                      //receive byte as an integer
  }
  rxb = Wire.read();                                        //receive byte as an integer
  if (debug == true) {
    if (rxa == 0 && rxb == 0) Serial.println("Scanning halted by Master");
    if (rxa == 1 && rxb == 0) Serial.println("Scanning restarted by Master");
  }
...
void masterSend(byte rxa, byte rxb, byte ch) {
  ...
  switch (rxa) {
    ...
    default:
      ...
      b = rxa;
      c = rxb;
      if (debug == true) Serial.printf("TX Controller\t%d\t%d\t%d\n\r", ch, b + nrpnOffset, c);
      if (b > 15) {  //If a piston is pressed force an analogue controller update
        Oswell[0] = 128;
        Oswell[1] = 128;
        Oswell[2] = 128;
        Oswell[3] = 128;
      }
      uint16_t n = rxa + nrpnOffset;            //Calculate the NRPN register number, the first 4 are reservered for swell pedals
          ...
          if (par1 == 6) {                        //NRPN value commmand
            ...
            if (n >= nrpnOffset && n < nrpnOffset + 36) {
              ...
              for (int iii = 0; iii < i2cmax; iii++) {
                if (i2cchan[iii] == ch) {
                  if (debug == true) Serial.printf(">> Controller\t0x%x\t%d\t%d\n\r", i2cfile[iii], n, par2);
